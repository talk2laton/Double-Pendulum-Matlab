...
vid = VideoWriter('DBPLMImplicit_2','MPEG-4'); set(vid, 'FrameRate',30); 
open(vid); 
% Draw the support
xs = [-3, 0, 3]; F = [1, f2, f3];
...
T = linspace(0,15,451); 
opts = odeset('Mass',@(t, s)Mass(s, L1, L2, M1, M1));
[~, Y1] = ode45(@(t, s) Dynamics(s, G, L1, L2, M1, M1), T, s, opts);
opts = odeset('Mass',@(t, s)Mass(s, L1, L2, M1, M2));
[~, Y2] = ode45(@(t, s) Dynamics(s, G, L1, L2, M1, M2), T, s, opts);
opts = odeset('Mass',@(t, s)Mass(s, L1, L2, M1, M3));
[~, Y3] = ode45(@(t, s) Dynamics(s, G, L1, L2, M1, M3), T, s, opts);
States = {Y1, Y2, Y3};
for n = 1:451
    for i = 1:3
        s = States{i}(n, :);
...
function dydx = Dynamics(y, G, L1, L2, M1, M2)
    sd = sin(y(1) - y(3));
    dydx = [y(2); 
            L1*sd*y(2)^2-G*sin(y(3));
            y(4); 
            -M2*L2*sd*y(4)^2-(M1 + M2)*G*sin(y(1))];
end

function M = Mass(y, L1, L2, M1, M2)
    cd = cos(y(1) - y(3)); 
    M = [1,     0,       0,   0
         0,   L1*cd,     0,   L2
         0,     0,       1,   0
         0, (M1+M2)*L1,  0, M2*L2*cd];
end