...
vid = VideoWriter('DBPLMImplicit','MPEG-4'); set(vid, 'FrameRate',30); 
open(vid); 
% Draw the support
xs = [-3, 0, 3]; F = [1, f2, f3];
...
T = linspace(0,15,451);
[~, Y1] = ode45(@(t, s) Dynamics(s([1,3]), s([2,4]), G, L1, L2, M1, M1), T, s);
[~, Y2] = ode45(@(t, s) Dynamics(s([1,3]), s([2,4]), G, L1, L2, M1, M2), T, s);
[~, Y3] = ode45(@(t, s) Dynamics(s([1,3]), s([2,4]), G, L1, L2, M1, M3), T, s);
States = {Y1, Y2, Y3};
for n = 1:451
    for i = 1:3
        s = States{i}(n, :);
...
function dydx = Dynamics(y, yp, G, L1, L2, M1, M2)
    delta = y(1) - y(2); cd = cos(delta); sd = sin(delta);
    A = [L1*cd, L2; (M1+M2)*L1, M2*L2*cd ];
    C = [L1*sd; -M2*L2*sd];
    D = [-G*sin(y(2)); -(M1 + M2)*G*sin(y(1))];
    yp2 = yp.^2;
    ypp = A\(C.*yp2 + D);
